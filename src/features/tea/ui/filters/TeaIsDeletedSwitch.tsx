...
export function TeaIsDeletedSwitch() {
  useSignals();
  const id = useId();

  const isChecked = TeaFiltersStore.filter.isDeleted ?? false;
  const onCheckedChange = (value: boolean) => TeaFiltersStore.setIsDeleted(value || null);
